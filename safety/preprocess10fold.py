...
class preprocess_():
    ...
    def PROCESS_1_USE_DATA(self, connectomes1000_project = True,
                           SALD_project = False,
                           ADNI = False,
                           MTA = False,
                           MIGRAINE_True = False,
        ...
        #%%tenfold
        import pickle
        FOLD_NUM_SHAPE = self.labels['file_name'].shape[0]
        label_lab = np.arange(0, FOLD_NUM_SHAPE)
        np.random.shuffle(label_lab)
        
        def groupping(length, k = 10):
            div_val = length/k
            lower_val = length//k
            w_num = np.round_((div_val - lower_val) * k)
            out = [0]
            for idx in range(k):
                if(w_num>idx):
                    out.append(out[-1] + lower_val+1)
                else:
                    out.append(out[-1] + lower_val)
            return out
        
        grp = groupping(FOLD_NUM_SHAPE)
        
        self.train = dict()
        self.test = dict()
        for idx in range(len(grp)-1):
            
            self.test[str(idx)] = {'file_name' : self.labels['file_name'][label_lab[grp[idx]:grp[idx+1]]],
                                   'age' : self.labels['age'][label_lab[grp[idx]:grp[idx+1]]],
                                   'sex' : self.labels['sex'][label_lab[grp[idx]:grp[idx+1]]]}
            self.train[str(idx)] = {'file_name' : self.labels['file_name'][np.concatenate((label_lab[:grp[idx]], label_lab[grp[idx+1]:]))],
                                    'age' : self.labels['age'][np.concatenate((label_lab[:grp[idx]], label_lab[grp[idx+1]:]))],
                                    'sex' : self.labels['sex'][np.concatenate((label_lab[:grp[idx]], label_lab[grp[idx+1]:]))]}
        with open(self.var_TARGET_DIR_TFR + 'trainKFold.pickle', 'wb') as handle:
            pickle.dump(self.train, handle, protocol=pickle.HIGHEST_PROTOCOL)
        with open(self.var_TARGET_DIR_TFR + 'testKFold.pickle', 'wb') as handle:
            pickle.dump(self.test, handle, protocol=pickle.HIGHEST_PROTOCOL)
    #%%tenfold END
            
    def PROCESS_2_CREATE_TFRECORD(self, tfrecord_filename, adx):
        if (self.var_split_TRAIN_VALID == 0):
            self.writer = self._generate_TFRecord_(tfrecord_filename)
        else:
            self.writer_TRAIN = self._generate_TFRecord_(tfrecord_filename + "_TRAIN" + str(adx))
            self.writer_VALID = self._generate_TFRecord_(tfrecord_filename + "_VALID" + str(adx))
    
    def PROCESS_3_SAVE(self, adx):
        start = time.time()
        num_of_saved_files = 0
        for idx in range(self.train[str(adx)]['age'].shape[0]):
            
            splited_file_name = self.train[str(adx)]['file_name'][idx].split('.')[0].split('\\')[-1] # split the filename
            
            img = nib.load(self.train[str(adx)]['file_name'][idx]) #get nifti file
            ...
            label = self.train[str(adx)]['age'][idx]
            sex = self.train[str(adx)]['sex'][idx]
        ...
        print("\n{0} train files saved in TFRecords.".format(num_of_saved_files))
        
        start = time.time()
        num_of_saved_files = 0
        for idx in range(self.test[str(adx)]['age'].shape[0]):
            
            splited_file_name = self.test[str(adx)]['file_name'][idx].split('.')[0].split('\\')[-1] # split the filename
            
            img = nib.load(self.test[str(adx)]['file_name'][idx]) #get nifti file
            arr = np.array(img.dataobj)
            ...
            file_name = os.getcwd() + '\\' + self.val_TARGET_DIR_NPY + '\\'+splited_file_name
            label = self.test[str(adx)]['age'][idx]
            sex = self.test[str(adx)]['sex'][idx]
            np.save(file_name,arr)
            ...
            os.remove(file_name +'.npy')
            num_of_saved_files += 1
        print("\n{0} test files saved in TFRecords.".format(num_of_saved_files))
