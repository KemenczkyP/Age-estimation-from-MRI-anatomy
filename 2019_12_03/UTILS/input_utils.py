...
class manip():
    ...
    def scan_BB():
        ...
        data_sheet = pd.read_csv('D:\\Peti\\Biobank\\main_dataset\\main_dataset\\ukb24364.csv')
        normed = PU.manip.find_in_subdirs('D:\\Peti\\Biobank\\SALD_spm', 'anat\\wm*')
        
        for idx in range(len(normed)):
            # 1 male, 0 female
            # sex col id: 31-0.0
            # age:
                    # 34-0.0 year of brith
                    # 52-0.0 month of birth
                    # 53-2.0 assessment
            # eid: id
            
            f_name = normed[idx]
            file_names.append(f_name)# save file name
            id_ = f_name.split('.')[0].split('\\')[-1].split('wm')[1].split('_')[0]
            row = data_sheet.loc[data_sheet['eid'] == int(id_)]
            sex.append(1-row['31-0.0'].values) # save age (in database male is 0, but for me it is 1)
            
            asse = pd.to_datetime(row['53-2.0']).values
            m = row['52-0.0'].values
            y = row['34-0.0'].values
            str_ = str(y[0]) + '/' + str(m[0])
            birth = pd.to_datetime(pd.Series(str_)).values
            diff = asse-birth
            ds = (diff[0]).astype('timedelta64[D]')
            days = ds.astype(int)
            age.append(days / 365.25)
        d['age'] = np.array(age)
        d['file_name'] = np.array(file_names)
        d['sex'] = np.reshape(np.array(sex),(-1))
        return d
    ...
    def scan_MRA():
        ...
        data_sheet = pd.read_csv('D:\\Peti\\Aging\\data\\MR_artefact\\dicominfos.csv')
        normed = PU.manip.find_in_subdirs('D:\\Peti\\Aging\\data\\MR_artefact\\SALD_spm', 'anat\\wm*')
        ...
        for idx in range(len(normed)):
            ...
            file_names.append(f_name)# save file name
            try:
                id_ = f_name.split('.')[0].split('\\')[-1].split('wm')[1].split('_')[-1]
                row = data_sheet.loc[data_sheet['head1'] == id_]
                
                if(row['head3'].values[0] == 'M'):
                    sex.append(1)
                elif(row['head3'].values[0] == 'F'):
                    sex.append(0)
                else:
                    print('unknown gender')
            except:
                id_ = f_name.split('.')[0].split('\\')[-1].split('wm')[1].split('_')[-2]
                row = data_sheet.loc[data_sheet['head1'] == id_]
            ...
            y = row['head2'].values[0]
            year = int(y.split('Y')[0])
            age.append(year)
        d['age'] = np.array(age)
        d['file_name'] = np.array(file_names)
        d['sex'] = np.reshape(np.array(sex),(-1))
