...
def cnn_model_fn(features, labels, mode):
    ...
    with tf.device('/gpu:0'):
        ...
        logits = ANN.net(input_layer, sexs, num_class, drop_out_rate,training = 1)
...
def filenames(TRAIN_VALID_TEST, data_dir):
    """Return filenames for dataset."""
    if TRAIN_VALID_TEST[0]:
      return [
          os.path.join(data_dir, 'train_all_data')
          for i in range(1)]
    if TRAIN_VALID_TEST[2]:
        return [
          os.path.join(data_dir, 'test_NOTmigrene_data')
          for i in range(1)]
    if TRAIN_VALID_TEST[1]:
      return [
          os.path.join(data_dir, 'valid_all_data')
          for i in range(1)]
    else:
...
def input_fn( data_dir = _DATA_DIR, batch_size=batch_size, num_epochs=1):
    ...
    dataset = tf.data.Dataset.from_tensor_slices(filenames([True, False, False], data_dir))
...
def input_valid_fn(data_dir = _DATA_DIR, batch_size=batch_size, num_epochs=1):
    """Input function which provides batches."""
    dataset = tf.data.Dataset.from_tensor_slices(filenames([False,True,False], data_dir))
    dataset = dataset.flat_map(tf.data.TFRecordDataset)
    dataset = dataset.map(lambda value: record_parser(value), num_parallel_calls=5)
    dataset = dataset.shuffle(buffer_size = batch_size*100)
    dataset = dataset.prefetch(batch_size)
...
with tf.Session(config=config) as sess:
    ...
    mnist_classifier = tf.estimator.Estimator(
            ...
            model_dir=os.getcwd() +"/tmp/mnist_convnet_model\\train_modifiedNET_10_MAE",
