...
class preprocess_():
    def __init__(self,
        ...
        info_csv = pd.read_csv('.\\data_mni\\info_file.csv')
        self.info_csv = info_csv.to_dict(orient = 'list')
    ...
    def PROCESS_2_USE_DATA(self, connectomes1000_project = True,
        ...
        if(MIGRAINE_True):
            self.labels = dict()
            df = pd.read_csv('.\\data_mni\\labels_migP.csv')
            d = df.to_dict(orient='list')
            self.labels['age'] = np.array(d['1'])
            self.labels['sex'] = np.array(d['3'])
            self.labels['file_name'] = np.array(d['2'])
        
        if(MIGRAINE_False):
            self.labels = dict()
            df = pd.read_csv('.\\data_mni\\labels_migN.csv')
            d = df.to_dict(orient='list')
            self.labels['age'] = np.array(d['1'])
            self.labels['sex'] = np.array(d['3'])
            self.labels['file_name'] = np.array(d['2'])
...
preprocesser = preprocess_(MRI_volume_size = [79,95,79],
                           ...
                           TARGET_DIR_TFR = 'TEMP_DATA/tf_record_folder/2019_09_11/',
                           split_TRAIN_VALID = 0)


preprocesser.PROCESS_1_CREATE_TFRECORD('Mig_P_unified')

l = preprocesser.PROCESS_2_USE_DATA(connectomes1000_project = False,
                                SALD_project = False,
                                ADNI = False,
                                MTA = False,
                                MIGRAINE_True = True,
