...
class Data_Iterator:
    ...
    def GetData(self):
        ...
        if (self.current_pointer + self.batch_size >= len(self.current_shuffle)):
            ...
            images = self.ReadNumpy(
                self.datadict['file_name'].iloc[self.current_shuffle[self.current_pointer:]].to_numpy())
            ...
            labels = self.datadict['age'].iloc[
                self.current_shuffle[self.current_pointer: self.current_pointer + self.batch_size]].to_numpy()
            sexs = self.datadict['sex'].iloc[
                self.current_shuffle[self.current_pointer: self.current_pointer + self.batch_size]].to_numpy()
            images = self.ReadNumpy(self.datadict['file_name'].iloc[self.current_shuffle[
                                                                    self.current_pointer: self.current_pointer + self.batch_size]].to_numpy())
